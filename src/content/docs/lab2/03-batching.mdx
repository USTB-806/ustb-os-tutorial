...
- `ecall`：触发执行环境调用，从低特权级陷入高特权级处理流程；
- `sret`：在 RISC-V S 模式下用于从内核返回到先前特权级（通常是 U 模式）。
...
| wfi                | 处理器在空闲时进入低功耗状态等待中断。在U模式下执行会触发非法指令异常 |
| sfence.vma         | 刷新TLB缓存。在U模式下执行会触发非法指令异常                 |
| 访问S模式CSR的指令 | 通过访问 [sepc/stvec/scause/sscratch/stval/sstatus/satp等CSR](https://rcore-os.gitcode.host/rCore-Tutorial-Book-v3/chapter2/4trap-handling.html#term-s-mod-csr) 来改变系统状态。在U模式下执行会触发非法指令异常 |
...
>
> `lazy_static!` 宏提供了全局变量的运行时初始化能力。借助 `UPSafeCell`/`RefCell` 这类内部可变性容器，我们就能在尽量避免 `static mut` 的前提下管理全局状态。
...
> 为什么这里需要 `fence.i`：我们刚把新的应用指令拷贝到 `0x80400000`，但 CPU 的 i-cache 里可能仍缓存旧内容。`fence.i` 会让后续取指看到最新内存内容，避免“代码已写入但仍执行旧指令”的问题。
